...
class LDCCompiler : Compiler {
	...
	final string version_(in ref BuildPlatform platform)
	{
		import std.regex : matchFirst, ctRegex;

		if (versionCache.length) return versionCache;

		auto r = ctRegex!(`^LDC - the LLVM D compiler \((\d+\.\d+\.\d+(-\w+)?)\)`);
		invokeTool([ platform.compilerBinary, "--version" ], (int code, string output) {
			...
			enforce(c, "Could not parse LDC version");
			assert(c.length >= 2);
			c.popFront();
	...
	string frontendVersion(in ref BuildPlatform platform)
	{
		...
		auto r = ctRegex!(`based on DMD v(\d+\.\d+\.\d+(-\w+)?)`);
		invokeTool([ platform.compilerBinary, "--version" ], (int code, string output) {
			enforce(code == 0, "Could not run `"~platform.compilerBinary~" --version`");
			auto c = matchFirst(output, r);
			enforce(c, "Could not parse LDC front-end version");
			assert(c.length >= 2);
			c.popFront();
